...
const Search = ({ monsters, setDisplayedMonsters }) => {
  ...
  const handleSearchClick = () => {
    const NameResults = monsters.filter((monster) =>
      monster.name.toLowerCase().includes(searchQuery.toLowerCase())
    );
    const LocationResults = monsters.filter((monster) => {
      if (monster.common_locations === null) {
        return false;
      }
      return monster.common_locations
        .join(' ')
        .toLowerCase()
  ...
  return (
    <div className="search-bar">
      <input
        type="text"
        placeholder="Search Monster or Location!"
        value={searchQuery}
